  ...
  document.addEventListener('change', (e) => {
    if (e.target.classList.contains('table-completion-date-input')) {
      ...
      if (tr) {
        ...
        // cells[2] = Trạng thái
        if (cells[2]) {
          cells[2].innerHTML = `<span class="status-cell ${task.status}">${getStatusText(task.status)}</span>`;
        }
        // cells[7] = Tiến độ
    ...
    if (e.target.classList.contains('note-completion-date-input')) {
      ...
      if (tr) {
        const cells = tr.querySelectorAll('td');
        if (cells[6]) {
          cells[6].innerHTML = `<span class="badge badge-${task.status}">${getStatusText(task.status)}</span>`;
        }
      }
      ...
      google.script.run
        .withSuccessHandler(function(res) {
          if (res && res.success) {
            ...
            loadData(true);
            showNotification('Lỗi khi cập nhật: ' + (res ? res.message : 'Unknown'), 'error');
          }
        })
        .withFailureHandler(function(err) {
          loadData(true);
          showNotification('Lỗi kết nối: ' + err, 'error', true);
        })
        .updateNoteTask(task);
    }
      ...
      google.script.run
        .withSuccessHandler(function(res) {
          if (res && res.success) {
            ...
            loadData(true);
            showNotification('Lỗi khi cập nhật ghi chú: ' + (res ? res.message : 'Unknown'), 'error');
          }
        })
        .withFailureHandler(function(err) {
          loadData(true);
...
function updateTaskElementInUI(taskData) {
  ...
  if (listViewRow) {
    ...
    if (cells.length >= 10) {
      // Tiêu đề
      cells[0].querySelector('.task-title-cell').textContent = taskData.title;
      
      // Mô tả
      cells[1].textContent = taskData.description || '';
      
      // Trạng thái
      cells[2].innerHTML = `<span class="status-cell ${taskData.status}">${getStatusText(taskData.status)}</span>`;
...
function calculateAndRenderStats() {
  ...
  if (jobStatsTbody && jobStatsTfoot) {
    ...
    if (!keyword) {
      jobStatsTbody.innerHTML = '<tr><td colspan="6" class="empty-cell" style="text-align: center; padding: 20px; color: var(--text3);"><i class="fas fa-keyboard" style="font-size: 2rem; margin-bottom: 10px; display: block; opacity: 0.3;"></i>Vui lòng nhập từ khoá công việc để thống kê sản lượng</td></tr>';
    } else {
      ...
      if (matchingTasks.length === 0) {
        jobStatsTbody.innerHTML = `<tr><td colspan="6" class="empty-cell" style="text-align: center; padding: 20px; color: var(--text3);"><i class="fas fa-circle-info" style="font-size: 2rem; margin-bottom: 10px; display: block; opacity: 0.3;"></i>Không tìm thấy công việc nào khớp từ khoá "${keyword}" trong tháng này</td></tr>`;
      } else {
        // Gom nhóm và cộng tổng các công việc trùng tên của cùng tổ, đồng thời lọc theo tổ được chọn
        ...
        matchingTasks.forEach(task => {
          ...
          // Lấy tên người phụ trách từ danh sách assignees
          ...
          } else if (assigneeNames && !groupedSummary[key].assigneeNames.includes(assigneeNames)) {
            groupedSummary[key].assigneeNames += ', ' + assigneeNames;
          }
        ...
        // Sắp xếp các hàng theo Tên Tổ, sau đó theo Tiêu đề công việc
        reportRows.sort((a, b) => {
          if (a.teams !== b.teams) {
            return a.teams.localeCompare(b.teams, 'vi');
          }
        ...
        reportRows.forEach(row => {
          const rate = row.plan > 0 ? Math.round((row.actual / row.plan) * 100) : 0;
          grandTotalPlan += row.plan;
        ...
        if (rowsHtml === '') {
          jobStatsTbody.innerHTML = '<tr><td colspan="6" class="empty-cell" style="text-align: center; padding: 20px; color: var(--text3);"><i class="fas fa-circle-question" style="font-size: 2rem; margin-bottom: 10px; display: block; opacity: 0.3;"></i>Các công việc tìm thấy chưa được gán tổ phụ trách hoặc không có số liệu sản lượng</td></tr>';
        } else {
          ...
          jobStatsTfoot.innerHTML = `
            <tr style="border-top: 2px solid rgba(255,255,255,0.15); background: rgba(255,255,255,0.04); font-weight: 700;">
              <td style="padding: 12px; text-transform: uppercase; font-size: 0.85rem; letter-spacing: 0.5px;" colspan="3"><i class="fas fa-calculator" style="margin-right: 6px; color: #f59e0b;"></i>Tổng cộng</td>
